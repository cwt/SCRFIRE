...
screen  = curses.initscr() #Initialize the library & Return a window object which represents the whole screen.
width   = screen.getmaxyx()[1] #variabling the screen reso to be flexible to its screen max  
...
for i in range(size+width+1): b.append(0) #adds elements to the end of the list
...
while 1:
        ...
        for i in range(size):
                ...
                if(i<size-1):   screen.addstr(  int(i/width),     #displays a string at the current cursor location in the
                                                i%width,
        ...
        if (screen.getch()!=-1): break    #create a new window but you don't enable the keypad for it
